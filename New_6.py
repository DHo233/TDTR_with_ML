...
def synthetic_data(a, b, num_examples):
    ...
    # phase_noise = torch.randn(100, 1)*0.005 + 0.05
    # phase = phase * (1 + phase_noise)                                           #添加5%左右的噪声
...
freq, phase = synthetic_data(coef1, coef2, 100)                                 #这里是生成数据集
...
for epoch in range(num_epochs):
    for X, Y in data_iter:
        ...
        coefx[:,1] = torch.abs( coefx[:,1] * scale_coef2 )
        print(coefx)
        loss = 0
        for i in range(batch_size):
            ...
            # print(x[i])
            loss1 = loss_fn(mark1, two_materials_simple_2layer(x[i] * 2 * torch.pi, coefx[0][0], coefx[0][1]))
            loss = loss + loss1
            # print(loss)
        loss = loss / batch_size
        print("loss为",loss)
        if(loss <= 0.01): sys.exit()
