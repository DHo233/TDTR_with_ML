...
for epoch in range(num_epochs):
    for X, Y in data_iter:
        ...
        coefx[:,1] = torch.abs( coefx[:,1] * noise_coef1 )                      #2.28:用noise_coef矫正噪声的数量级 
        coefx[:,3] = torch.abs( coefx[:,3] * noise_coef2 )
        print(coefx)
        loss = 0
        for i in range(batch_size):
            ...
            # print(x[i])
            loss1 = loss_fn(mark1, 
                            two_materials_simple_2layer(x[i] * 2 * torch.pi, coefx[0][0] - coefx[0][1], coefx[0][2] - coefx[0][3]))\
                            + 1e4 * torch.abs(coefx[0][1]) + 1e11 * torch.abs(coefx[0][3])
            loss = loss + loss1
            # print(loss)
